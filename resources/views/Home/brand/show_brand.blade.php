...
@section('content')
    <div class="features_items"><!--features_items-->
        ...
        @foreach($show_brand as $key=>$value_brand)
            <a href="{{URL::to('/product-details/'.$value_brand->product_id)}}">
                <div class="col-sm-4">
                    <div class="product-image-wrapper">
                        <div class="single-products">
                            <div class="productinfo text-center">
                                <img src="{{URL::to('style/uploads/product/'.$value_brand->product_image)}}"width="120" height="150" alt="" />
                                <h2>{{number_format($value_brand->product_price)}} VNĐ</h2>
                                <p>{{$value_brand->product_name}}</p>
                                <a href="{{URL::to('/product-details/'.$value_brand->product_id)}}" class="btn btn-default add-to-cart"><i class="fa fa-shopping-cart"></i>Show Details</a>
                            </div>
                        </div>
                        <div class="choose">
